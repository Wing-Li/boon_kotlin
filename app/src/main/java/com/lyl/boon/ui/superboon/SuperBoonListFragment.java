...
public class SuperBoonListFragment extends BaseRecyclerFragment<SuperGalleryEntity.ListBean> {
    ...
    public void onAttach(Context context) {
        ...
        typeId = getArguments().getInt(SuperBoonFragment.Companion.getSUPER_TYPE());
    }
    ...
    protected void initData() {
        setMData(new ArrayList<SuperGalleryEntity.ListBean>());
        setMAdapter(new SuperBoonListAdapter(getHolder(), getMData(), R.layout.item_image_v));
    }

    @Override
    protected void setSubscribe(Observer observer) {
        int count = getPage() * ROWS;
        Network.INSTANCE.getTngou().getGalleryList( typeId, count ).map(new Func1<SuperGalleryEntity, List<SuperGalleryEntity.ListBean>>() {
    ...
    protected void ItemClickListener(View itemView, int viewType, int position) {
        SuperGalleryEntity.ListBean galleryEntiry = (SuperGalleryEntity.ListBean) getMAdapter().getItem( position );
